...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            this.accumulator = new RecordAccumulator(logContext,
                    ...
                    lingerMs(config),//生产者在缓冲区逗留的毫秒数 默认是0 如果生产者在此时间内存满batchSize大小的缓冲区那么会立刻发送否则此时间超时后发送
                    retryBackoffMs,
                    deliveryTimeoutMs,
                    partitionerConfig,
                    metrics,
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
        ...
        //0 不等服务器确认 1 leader 等待落盘 -1(all) 等待所有isr集合中的副本落盘
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            //分区操作 未设置->RecordAccumulator org.apache.kafka.clients.producer.internals.BuiltInPartitioner
            //
    ...
    private int partition(ProducerRecord<K, V> record, byte[] serializedKey, byte[] serializedValue, Cluster cluster) {
        ...
        if (serializedKey != null && !partitionerIgnoreKeys) {//key不为null且不忽略key(partitioner.ignore.keys为false)
