...
public class RecordAccumulator {
            ...
            while (true) {
                ...
                synchronized (dq) {
                    ...
                    if (partitionChanged(topic, topicInfo, partitionInfo, dq, nowMs, cluster))
                        continue;
                    //尝试往已有in-progress record batch内存中添加数据
                ...
                //没有in-progress record batch内存 需要先分配一块
                if (buffer == null) {
    ...
    private RecordAppendResult tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers,
        ...
        ProducerBatch last = deque.peekLast();
        if (last != null) {
            int initialBytes = last.estimatedSizeInBytes();//当前batch大小
            FutureRecordMetadata future = last.tryAppend(timestamp, key, value, headers, callback, nowMs);
            if (future == null) {//添加失败说明batch满了 需要关闭添加记录的操作
                last.closeForRecordAppends();
            } else {
                int appendedBytes = last.estimatedSizeInBytes() - initialBytes;//实际添加的大小=当前大小-添加前大小
    ...
    private long batchReady(long nowMs, boolean exhausted, TopicPartition part, Node leader,
        ...
        if (!readyNodes.contains(leader) && !isMuted(part)) {
            //是否重试了 如果重试timeToWaitMs为重试时间 否则timeToWaitMs=lingerMs
            ...
            if (sendable && !backingOff) {//如果可发送并且没有重试过
    ...
    private long partitionReady(Cluster cluster, long nowMs, String topic,
        ...
        for (Map.Entry<Integer, Deque<ProducerBatch>> entry : batches.entrySet()) {
            ...
            Node leader = cluster.leaderFor(part);//获取partition的leader副本的node
            ...
            if (leader == null) {
                ...
                if (partitionAvailabilityTimeoutMs > 0) {//partition超时
    ...
    public Map<Integer, List<ProducerBatch>> drain(Cluster cluster, Set<Node> nodes, int maxSize, long now) {
        ...
        for (Node node : nodes) {
            ...
            batches.put(node.id(), ready);//添加数据 key node id / value batches
        }
        return batches;
    }
