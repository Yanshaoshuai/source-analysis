...
public class BufferPool {
    ...
    private final long totalMemory;//最大内存
    private final int poolableSize;//等于batchSize
    private final ReentrantLock lock;
    //内存池底层 使用ByteBuffer队列实现内存池
    private final Deque<ByteBuffer> free;//回收过的内存 一个节点poolableSize
    private final Deque<Condition> waiters;
    /** Total available memory is the sum of nonPooledAvailableMemory and the number of byte buffers in free * poolableSize.  */
    private long nonPooledAvailableMemory;//未使用过的内存
    ...
    public ByteBuffer allocate(int size, long maxTimeToBlockMs) throws InterruptedException {
        ...
        try {
            ...
            if (size == poolableSize && !this.free.isEmpty())//正好是poolableSize大小 从回收队列弹出一个节点供使用
                return this.free.pollFirst();

            // now check if the request is immediately satisfiable with the
            // memory on hand or if we need to block
            int freeListSize = freeSize() * this.poolableSize;//计算剩余大小
            if (this.nonPooledAvailableMemory + freeListSize >= size) {//available memory=nonPooledAvailableMemory + freeListSize
                                                                        //总大小是否大于等于size
    ...
    private ByteBuffer safeAllocateByteBuffer(int size) {
        ...
        try {
            ...
            error = false;
            return buffer;
        } finally {//如果发生错误会回滚之前的nonPooledAvailableMemory
            if (error) {
                this.lock.lock();
                try {
                    this.nonPooledAvailableMemory += size;
    ...
    private void freeUp(int size) {
        //如果回收列表不为空且未池化的内存不够 会将回收列表中的内存加到未池化内存
